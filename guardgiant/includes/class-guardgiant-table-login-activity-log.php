...
class Guardgiant_Table_Login_Activity_Log extends WP_List_Table
{
    ...
    private function table_data($per_page = GUARDGIANT_DEFAULT_ITEMS_PER_PAGE_ON_ACTIVITY_LOG, $page_number = 1)
    {
        ...
        // attempt_time, trusted_device, ip_address, ip_location, username, result_code, result_description

        if ( ! empty( $_REQUEST['orderby'] ) ) {
            if (in_array($_REQUEST['orderby'], ["attempt_time", "trusted_device", "ip_address", "ip_location", "username", "result_code", "result_description"])) {
